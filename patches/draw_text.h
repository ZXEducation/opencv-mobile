...
static void get_text_drawing_size(const char *text, int fontpixelsize, int *w,
                                  int *h, const FontFace &ff = FontFace()) {
  *w = 0;
  *h = 0;
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (ff.d->get_glyph_bitmap(ch)) {
      line_w += fontpixelsize * (ch < 128 ? 1 : 2);
    } else {
...
static void resize_bilinear_font(const unsigned char *font_bitmap,
                                 unsigned char *resized_font_bitmap,
                                 int fontpixelsize, int fullwidth = 0) {
  ...
  for (int dy = 0; dy < h; dy++) {
      ...
      if (sy >= srch - 1) {
        ...
        for (int dx = 0; dx < w; dx++) {
          ...
          if (sx < 0) {
            ...
            S1p1 =
                sx % 2 == 0 ? (S1[sx / 2] & 0xf0) >> 4 : S1[sx / 2 + 1] & 0x0f;
          }
      ...
      if (sy >= srch - 1) {
        ...
        for (int dx = 0; dx < w; dx++) {
          ...
          if (sx < 0) {
            ...
            S0p1 =
                sx % 2 == 0 ? (S0[sx / 2] & 0xf0) >> 4 : S0[sx / 2 + 1] & 0x0f;
          }
    ...
    if (dy + 1 < h && yofs[dy + 1] == sy) {
      ...
      for (; dx + 15 < w; dx += 16) {
        ...
        int16x8_t _acc00 =
            vaddq_s16(vqdmulhq_s16(_r00, _b0), vqdmulhq_s16(_r10, _b1));
        int16x8_t _acc01 =
            vaddq_s16(vqdmulhq_s16(_r01, _b0), vqdmulhq_s16(_r11, _b1));
        int16x8_t _acc10 =
            vaddq_s16(vqdmulhq_s16(_r00, _b2), vqdmulhq_s16(_r10, _b3));
        int16x8_t _acc11 =
            vaddq_s16(vqdmulhq_s16(_r01, _b2), vqdmulhq_s16(_r11, _b3));
        uint8x16_t _Dp0 =
            vcombine_u8(vqrshrun_n_s16(_acc00, 3), vqrshrun_n_s16(_acc01, 3));
        uint8x16_t _Dp1 =
            vcombine_u8(vqrshrun_n_s16(_acc10, 3), vqrshrun_n_s16(_acc11, 3));
        vst1q_u8(Dp0, _Dp0);
        vst1q_u8(Dp1, _Dp1);
        Dp0 += 16;
        Dp1 += 16;
      ...
      for (; dx + 7 < w; dx += 8) {
        ...
        int16x8_t _acc0 =
            vaddq_s16(vqdmulhq_s16(_r0, _b0), vqdmulhq_s16(_r1, _b1));
        int16x8_t _acc1 =
            vaddq_s16(vqdmulhq_s16(_r0, _b2), vqdmulhq_s16(_r1, _b3));
      ...
      for (; dx + 15 < w; dx += 16) {
        ...
        __m128i _acc00 = _mm_add_epi16(_mm_mulhi_epi16(_r00, _b0),
                                       _mm_mulhi_epi16(_r10, _b1));
        __m128i _acc01 = _mm_add_epi16(_mm_mulhi_epi16(_r01, _b0),
                                       _mm_mulhi_epi16(_r11, _b1));
        __m128i _acc10 = _mm_add_epi16(_mm_mulhi_epi16(_r00, _b2),
                                       _mm_mulhi_epi16(_r10, _b3));
        __m128i _acc11 = _mm_add_epi16(_mm_mulhi_epi16(_r01, _b2),
                                       _mm_mulhi_epi16(_r11, _b3));
      ...
      for (; dx + 7 < w; dx += 8) {
        ...
        __m128i _acc0 =
            _mm_add_epi16(_mm_mulhi_epi16(_r0, _b0), _mm_mulhi_epi16(_r1, _b1));
        __m128i _acc1 =
            _mm_add_epi16(_mm_mulhi_epi16(_r0, _b2), _mm_mulhi_epi16(_r1, _b3));
      ...
      for (; dx < w; dx++) {
        ...
        *Dp0++ = (unsigned char)(((short)((b0 * s0) >> 16) +
                                  (short)((b1 * s1) >> 16) + 2) >>
                                 2);
        *Dp1++ = (unsigned char)(((short)((b2 * s0) >> 16) +
                                  (short)((b3 * s1) >> 16) + 2) >>
                                 2);
      }
      ...
      for (; dx + 15 < w; dx += 16) {
        ...
        int16x8_t _acc1 =
            vaddq_s16(vqdmulhq_s16(_r01, _b0), vqdmulhq_s16(_r11, _b1));
        uint8x16_t _Dp =
            vcombine_u8(vqrshrun_n_s16(_acc0, 3), vqrshrun_n_s16(_acc1, 3));
        vst1q_u8(Dp, _Dp);
        Dp += 16;
        rows0p += 16;
        rows1p += 16;
      ...
      for (; dx + 15 < w; dx += 16) {
        ...
        __m128i _r10 = _mm_loadu_si128((const __m128i *)rows1p);
        __m128i _r11 = _mm_loadu_si128((const __m128i *)(rows1p + 8));
        __m128i _acc0 = _mm_add_epi16(_mm_mulhi_epi16(_r00, _b0),
                                      _mm_mulhi_epi16(_r10, _b1));
        __m128i _acc1 = _mm_add_epi16(_mm_mulhi_epi16(_r01, _b0),
                                      _mm_mulhi_epi16(_r11, _b1));
        _acc0 = _mm_srai_epi16(_mm_add_epi16(_acc0, _v2), 2);
        _acc1 = _mm_srai_epi16(_mm_add_epi16(_acc1, _v2), 2);
      ...
      for (; dx < w; dx++) {
        ...
        *Dp++ = (unsigned char)(((short)((b0 * s0) >> 16) +
                                 (short)((b1 * s1) >> 16) + 2) >>
                                2);
      }
...
static void draw_text_c1(unsigned char *pixels, int w, int h, int stride,
                         const char *text, int x, int y, int fontpixelsize,
                         unsigned int color, const FontFace &ff) {
  const unsigned char *pen_color = (const unsigned char *)&color;

  unsigned char *resized_font_bitmap =
      new unsigned char[fontpixelsize * fontpixelsize * 4];
    ...
    if (font_bitmap) {
      ...
      for (int j = ystart; j < yend; j++) {
        const unsigned char *palpha = resized_font_bitmap +
                                      (j - cursor_y) * fontpixelsize + xstart -
                                      cursor_x;
        unsigned char *p = pixels + stride * j + xstart;
...
static void draw_text_c3(unsigned char *pixels, int w, int h, int stride,
                         const char *text, int x, int y, int fontpixelsize,
                         unsigned int color, const FontFace &ff) {
  const unsigned char *pen_color = (const unsigned char *)&color;
...
static void draw_text_c4(unsigned char *pixels, int w, int h, int stride,
                         const char *text, int x, int y, int fontpixelsize,
                         unsigned int color, const FontFace &ff) {
  const unsigned char *pen_color = (const unsigned char *)&color;
...
static void draw_text_c1(unsigned char *pixels, int w, int h, const char *text,
                         int x, int y, int fontpixelsize, unsigned int color,
                         const FontFace &ff = FontFace()) {
  return draw_text_c1(pixels, w, h, w, text, x, y, fontpixelsize, color, ff);
}

static void draw_text_c3(unsigned char *pixels, int w, int h, const char *text,
                         int x, int y, int fontpixelsize, unsigned int color,
                         const FontFace &ff = FontFace()) {
...
static void draw_text_c4(unsigned char *pixels, int w, int h, const char *text,
                         int x, int y, int fontpixelsize, unsigned int color,
                         const FontFace &ff = FontFace()) {
